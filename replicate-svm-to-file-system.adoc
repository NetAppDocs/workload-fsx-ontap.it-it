...
[role="lead"]
La replica di una VM di storage su un altro file system FSx for ONTAP in NetApp Workload Factory fornisce un livello protettivo di accesso ai dati in caso di perdita di dati.  Questa operazione replica tutti i volumi in una VM di archiviazione su un altro file system FSx for ONTAP .
...
. Da *FSx for ONTAP*, seleziona il menu delle azioni del file system con la VM di archiviazione da replicare, quindi seleziona *Gestisci*.
. Nella panoramica del file system in VM di archiviazione, selezionare *Gestisci*.
. Nella schermata Gestisci VM di archiviazione, seleziona il menu delle azioni della VM di archiviazione per cui replicare una SVM, quindi seleziona *Azioni avanzate* > *Replica VM di archiviazione*.
...
+
_Auto_ è il criterio di suddivisione in livelli predefinito quando si crea un volume utilizzando l'interfaccia utente Workload Factory FSx for ONTAP .  Per ulteriori informazioni sulle politiche di suddivisione in livelli del volume, fare riferimento alink:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/volume-storage-capacity.html#data-tiering-policy["Capacità di storage dei volumi"^] nella documentazione di AWS FSx per NetApp ONTAP .
