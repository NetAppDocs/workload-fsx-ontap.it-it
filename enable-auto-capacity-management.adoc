...
summary: Abilitando questa funzionalità, Workload Factory aggiunge automaticamente spazio di archiviazione incrementale a un file system FSx for ONTAP man mano che le esigenze di capacità cambiano nel tempo. 
---
...
[role="lead"]
Abilitando la gestione automatica della capacità, NetApp Workload Factory aggiunge automaticamente storage incrementale a un file system FSx for ONTAP man mano che le esigenze di capacità cambiano nel tempo.  Inoltre, l'abilitazione di questa funzionalità elimina la necessità di monitorare manualmente la capacità e riduce il rischio di letture e scritture di dati applicativi che hanno un impatto negativo.
...
* Per completare questa attività è necessario disporre di credenziali con autorizzazioni di lettura/scrittura in Workload Factory.
