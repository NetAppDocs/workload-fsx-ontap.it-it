...
.Volumi FlexVol
I volumi FlexVol sono contenitori logici che offrono flessibilità nella gestione dei dati, consentendone l'espansione, la contrazione, lo spostamento e la copia efficiente. Possono essere utilizzati in ambienti NAS e SAN.
...
La verifica dell'equilibrio della capacità di volume FlexGroup include la valutazione della disposizione attuale dei componenti.  Quando si ribilancia la capacità del volume, NetApp Workload Factory progetta un nuovo layout dei componenti con un numero pari di componenti per distribuire i dati in modo uniforme su tutte le coppie HA.  Il servizio esegue il piano di ribilanciamento che a sua volta migliora le operazioni di lettura e scrittura.
...
. Da *FSx for ONTAP*, seleziona il menu azioni del file system che contiene i volumi da ribilanciare, quindi seleziona *Gestisci*.
...
NOTE: Un amministratore di Workload Factory può<<Interrompere un'operazione di ribilanciamento del volume,smettere di riequilibrare>> durante l'operazione.
...
. Seleziona il menu image:workloads-menu-icon.png["L'icona del menu hamburger viene utilizzata per navigare tra carichi di lavoro quali storage, eda, ai, database, vmware e amministrazione."] e quindi seleziona *Archiviazione*.
. Dal menu Archiviazione, selezionare *FSx per ONTAP*.
. Da *FSx for ONTAP*, seleziona il menu azioni del file system che contiene il volume da ribilanciare, quindi seleziona *Gestisci*.
. Nella panoramica del file system, selezionare la scheda *volumi*.
. Dalla scheda Volumi, seleziona *Verifica bilanciamento volume* nella parte superiore della tabella.
