...
NOTE: AWS Secrets Manager non è supportato quando si utilizza un agente Console.
...
+
Tieni presente che a Lambda vengono concesse tre autorizzazioni quando avvii il modello CloudFormation. NetApp Workload Factory utilizza queste autorizzazioni quando usi i collegamenti.
...
link:links-overview.html#when-a-link-is-required["Ulteriori informazioni sui collegamenti"].
