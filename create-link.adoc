...
* Devi avere almeno un file system FSX per ONTAP in una workload factory. Per scoprire o creare file system FSX per ONTAP, devi disporre di un account AWS con permessi per le istanze di FSX per ONTAP e link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html#overview["aggiungere le credenziali in fabbrica del carico di lavoro"^] con permessi di sola lettura o lettura/scrittura per la gestione dello storage.
