...
Per la migrazione delle VM di storage, completare l'operazione di cutover immediatamente dopo aver creato una relazione di replica.
...
Dopo aver creato una relazione di replica per un caso d'uso di migrazione, è necessario eseguire il cutover della replica per completare la migrazione dei dati della VM di storage e delle impostazioni di configurazione al file system FSx for ONTAP di destinazione. Il cutover della replica migra in modo permanente i dati e le impostazioni di configurazione della VM di storage dal file system di origine al file system FSx for ONTAP di destinazione. Durante il cutover, i dati vengono replicati per l'ultima volta. La VM di storage di origine va offline all'inizio del cutover.
...
* link:initialize-replication.html["Inizializza la replica"] per una nuova relazione di replica.
